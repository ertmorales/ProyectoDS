...
namespace ApiSql.Models.tbl_cuenta_correntista_rel
{
    public class InsertData
    {
        ...
        public int insertData_cuenta_correntista_rel(string name)
        {
            ...
            using (var connection = Connection.ConnectionSql.getConnection())
            {
                ...
                foreach (var row in listajson)
                {
                    ...
                    SqlDataAdapter cmd = new SqlDataAdapter("select Id from dbo.cuenta_correntista_rel where Id = " + row.Id, connection);
                    ...
                    else
                    {
                        var detail = new TableModel()
                        {
                            ...
                            F_Cuenta_Correntista = row.F_Cuenta_Correntista,
                            F_Cuenta_Cta = row.F_Cuenta_Cta
                        };
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    using (var command = new SqlCommand())
                    {
                        ...
                        command.CommandText = "insert into dbo.cuenta_correntista_rel values (" +
                            "@Cuenta_Correntista," +
                            "@F_Cuenta_Correntista, " +
                            "@F_Cuenta_Cta" +
                            ")";
                        command.Parameters.Add("@Cuenta_Correntista", SqlDbType.Int);
                        command.Parameters.Add("@F_Cuenta_Correntista", SqlDbType.Int);
                        command.Parameters.Add("@F_Cuenta_Cta", SqlDbType.VarChar);
                        ...
                        try
                        {
                            foreach (var item in objectDetailList)
                            {
                                ...
                                command.Parameters["@F_Cuenta_Correntista"].Value = item.F_Cuenta_Correntista;
                                command.Parameters["@F_Cuenta_Cta"].Value = item.F_Cuenta_Cta;
